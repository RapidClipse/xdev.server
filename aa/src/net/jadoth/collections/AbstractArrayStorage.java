...
public abstract class AbstractArrayStorage
{
	...
	public static final <E> E queryElement(final E[] data, final int size, final Predicate<? super E> predicate, final E notFoundMarker)
	{
	...
	)
	{
		...
		return elements.applies(new Predicate<E>() { @Override public boolean test(final E e) {
			return containsSame(data, size, e);
		}});
	}
	...
	)
	{
		...
		if(length < 0){
			return elements.applies(new Predicate<E>() { @Override public boolean test(final E e) {
				return reverseContainsSame(data, offset, offset + length + 1, e);
			}});
		}
		return elements.applies(new Predicate<E>() { @Override public boolean test(final E e) {
			return forwardContainsSame(data, offset, offset + length, e);
		}});
	}
...
//			// directly check array against array without predicate function or method calls
//			return JaArrays.uncheckedContainsAll(
//				data,
//				length >= 0 ?offset        :offset+length+1,
//				length >= 0 ?offset+length :offset+1,
//				((AbstractSimpleArrayCollection<E>)samples).internalGetStorageArray(),
//				0,
//				samples.size(),
//				equalator
//			);
...
//				return reverseContains(data, offset, offset + length + 1, new IsCustomEqual<>(equalator, e));
//			}});
//		}
//		return samples.applies(new Predicate<E>() { @Override public boolean test(final E e) {
//			return forwardContains(data, offset, offset + length, new IsCustomEqual<>(equalator, e));
//		}});
//	}
	...
	public static final <E, C extends Procedure<? super E>> C copyTo(
		final E[] data,
		final int size,
		final C target
	)
	...
	public static final <E, C extends Procedure<? super E>> C rngCopyTo(
		final E[] data,
		final int size,
		final int offset,
		final int length,
	...
	public static final <E, C extends Procedure<? super E>> C forwardCopyTo(
		final E[] data,
		final int lowOffset,
		final int highBound,
		final C target
	...
	public static final <E, C extends Procedure<? super E>> C reverseCopyTo(
		final E[] data,
		final int highOffset,
		final int lowEnd,
		final C target
	...
	public static final <E, C extends Procedure<? super E>> C copySelection(
		final E[] data,
		final int size,
		final int[] indices,
		final C target
	)
	{
		...
		// actual copying
		for(int i = 0; i < length; i++){
			target.accept(data[indices[i]]); // manual loop to spare temporary variable
		}
	...
	public static final <E, C extends Procedure<? super E>> C intersect(
		final E[] data,
		final int size,
		final XGettingCollection<? extends E> samples,
		final Equalator<? super E> equalator,
	...
	public static final <E, C extends Procedure<? super E>> C except(
		final E[]                             data     ,
		final int                             size     ,
		final XGettingCollection<? extends E> samples  ,
		final Equalator<? super E>            equalator,
	...
	public static final <E, C extends Procedure<? super E>> C union(
		final E[]                             data     ,
		final int                             size     ,
		final XGettingCollection<? extends E> samples  ,
		final Equalator<? super E>            equalator,
	...
	public static final <E, C extends Procedure<? super E>> C rngIntersect(
		final E[]                             data     ,
		final int                             size     ,
		final int                             offset   ,
		final int                             length   ,
	...
	)
	{
		...
		if(samples instanceof AbstractSimpleArrayCollection<?>){
			final E[] array = AbstractSimpleArrayCollection.internalGetStorageArray((AbstractSimpleArrayCollection<?>)samples);
			final int arrayLength = Jadoth.to_int(samples.size());
			da: for(int di = offset; di != bound; di+=d){
				final E element = data[di];
				for(int i = 0; i < arrayLength; i++){
					if(equalator.equal(element, array[i])){
						target.accept(element);
	...
	public static final <E, C extends Procedure<? super E>> C rngExcept(
		final E[]                             data     ,
		final int                             size     ,
		final int                             offset   ,
		final int                             length   ,
	...
	public static final <E, C extends Procedure<? super E>> C rngUnion(
		final E[]                             data     ,
		final int                             size     ,
		final int                             offset   ,
		final int                             length   ,
	...
	)
	{
		...
		if(samples instanceof AbstractSimpleArrayCollection<?>){
			final E[] array = AbstractSimpleArrayCollection.internalGetStorageArray((AbstractSimpleArrayCollection<?>)samples);
			final int arrayLength = Jadoth.to_int(samples.size());
			ar: for(int i = 0; i < arrayLength; i++){
				final E sample = array[i];
				for(int di = offset; di != bound; di+=d){
					if(equalator.equal(data[di], sample)){
						continue ar;
		...
		samples.iterate(new Procedure<E>(){ @Override public void accept(final E e) {
			// local references to AIC fields
			final E[] data2 = data;
	...
	public static final <E, C extends Procedure<? super E>> C distinct(
		final E[] data,
		final int size,
		final C target
	)
	...
	public static final <E, C extends Procedure<? super E>> C rngDistinct(
		final E[] data,
		final int size,
		final int offset,
		final int length,
	...
	)
	{
		...
		try {
			for(int i = 0, idx; i < length; i++){
				target.accept(data[idx = indices[i]]); // manual loop to spare temporary variable
				data[idx] = removeMarker;
			}
		}
		catch (final ThrowBreak b) { /* abort iteration */}
		finally {
	...
	public static final <E> boolean removeOne(final E[] data, int size, final E sample, final Equalator<? super E> equalator)
	{
		for(int i = 0; i < size; i++){
			if(equalator.equal(data[i], sample)){
				if(i < --size){
	...
	)
	{
		...
		try{
			oldElements.iterate(new Procedure<U>(){ @Override public void accept(final U e) {
				final E[] data1 = data; // data is actually a field here
				for(int i = 0, len = size; i < len; i++){ // as is size
					if(data1[i] == e){
						data1[i] = marker;
						break;
					}
	...
	)
	{
		final int replaceCount;
		try{
			samples.iterate(new Procedure<U>(){ @Override public void accept(final U e) {
				final E[] data1 = data; // data is actually a field here
				for(int i = 0, len = size; i < len; i++){ // as is size
	...
	public static final <E> int modify(final E[] data, final int size, final Function<E, E> mapper)
	{
		int replaceCount = 0;
		for(int i = 0; i < size; i++){
			if(data[i] != (data[i] = mapper.apply(data[i]))){ // tricky :D
				replaceCount++;
			}
		}
		return replaceCount;
	...
	)
	{
		...
		int replaceCount = 0;
		for(int i = offset-d; i != endIndex;){
			if(data[i+=d] != (data[i] = mapper.apply(data[i]))){ // setting array right away is faster than buffer var.
				replaceCount++;
			}
		}
		return replaceCount;
	...
	{
		if(length == 0 || indexA == indexB) return;
	...
	public static final String toString(final Object[] data, final int size)
	{
		...
		final VarString vc = VarString.New((int)(size * 5.0f)).append('[');
		for(int i = 0; i < size; i++){
			vc.add(data[i]).add(',', ' ');
		}
	...
	)
	{
		...
		for(int i = 0; i < size; i++){
			vc.add(data[i]).append(seperator);
		}
		vc.deleteLast();
		return vc;
	}
	...
	)
	{
		...
		else if(seperator == null || seperator.isEmpty()){
			for(int i = 0; i < size; i++){
				vc.add(data[i]);
			}
		}
		...
		final char seperator)
	{
		...
		for(int i = offset-d; i != endIndex;){
			vc.add(data[i+=d]).append(seperator);
		}
		vc.deleteLast();
		return vc;
	}
	...
	{
		// "evil" magic number. Used exactly one time, so the constant was more clutter than it's worth
		if(size < 512){
			for(int i = 0; i < size; i++){
				final Object element = data[i];
	...
	public static final <E> boolean hasDistinctValues(final E[] data, final int size, final Equalator<? super E> equalator)
	{
		for(int i = 0; i < size; i++){
			final E element = data[i];
			for(int j = i + 1; j < size; j++){
	...
	)
	{
		...
		if(list instanceof RandomAccess){
			for(int i = 0; i < size; i++){
				if(!equalator.equal(data[i], list.get(i))){
					return false;
				}
			}
		}
			...
			int i = 0;
			for(final ListIterator<E> itr = list.listIterator(); itr.hasNext();){
