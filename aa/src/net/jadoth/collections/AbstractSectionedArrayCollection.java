...
public abstract class AbstractSectionedArrayCollection<E> extends AbstractArrayCollection<E>
{
	...
	/**
	 * Defines the array sections in which the collection's elements are organized by one or more pairs of indices in the
	 * order corresponding to the collection's logical order of its contained elements.
	 * <p>
	 * Examples:<br>
	 * The trivial example would be {0,size} in case of standard sized array collections.<br>
	 * An example for actual sectioning would be {i,array.length-1, 0,i-1} in case of a ring buffer list comprised of
	 * two sections: one ranging from index i to array.length-1 (with the oldest element located at i) and one
	 * ranging from 0 to i-1 (with the newest element located at i-1).
	 *
