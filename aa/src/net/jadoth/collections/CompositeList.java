...
public final class CompositeList<E> implements XGettingList<E>
{
	...
	public final E at(final int index)
	{
		...
		for(int c = 0, i = index; c < this.count; c++){
			if(i < lists[c].size()){
				return lists[c].at(i);
			}
			i -= lists[c].size();
	...
	{
		throw new net.jadoth.meta.NotImplementedYetError(); // FIX-ME XGettingCollection<E>#hasVolatileElements()
	}
...
//	}
	...
	@Override
	public final XGettingList<E> range(final int fromIndex, final int toIndex)
	{
