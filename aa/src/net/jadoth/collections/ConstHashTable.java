...
implements XImmutableTable<K,V>, HashCollection<K>, Composition, IdentityEqualityLogic
{
	...
	{
		throw new net.jadoth.meta.NotImplementedYetError(); // FIXME ConstHashTable#old()
	}

	@Override
	public net.jadoth.collections.types.XImmutableTable.Bridge<K, V> oldMap()
	{
	...
	public final Predicate<KeyValue<K,V>> predicateContainsEntry()
	{
		return new Predicate<KeyValue<K,V>>(){ @Override public boolean test(final KeyValue<K,V> entry) {
			...
			if((kv = ConstHashTable.this.getEntry(entry.key())) == null) return false;
	...
	{
		throw new net.jadoth.meta.NotImplementedYetError(); // FIX-ME ConstHashTable.Entries#range()
	}

	@Override
	public final XGettingEnum<KeyValue<K,V>> view(final int lowIndex, final int highIndex)
	{
		throw new net.jadoth.meta.NotImplementedYetError(); // FIX-ME ConstHashTable.Entries#view()
	}
	...
	public final KeyValue<K, V> seek(final KeyValue<K, V> sample)
	{
		...
		// search for element by hash
		for(ChainMapEntryLinkedStrongStrong<K,V> e = ConstHashTable.this.slots[System.identityHashCode(sample.key()) & ConstHashTable.this.range]; e != null; e = e.link){
			if(e.key() == sample.key()){
				return e;
			}
		}
	...
	{
		ConstHashTable.this.chain.copyToArray(0, ConstHashTable.this.size, target, offset);
		return target;
	}
	...
	{
		ConstHashTable.this.chain.copyToArray(offset, length, target, targetOffset);
		return target;
	}
	...
	public final class Keys implements XImmutableTable.Keys<K, V>, HashCollection<K>
	{
		...
		{
			throw new net.jadoth.meta.NotImplementedYetError(); // FIX-ME ConstHashTable.Keys#range()
		}

		@Override
		public final XGettingEnum<K> view(final int lowIndex, final int highIndex)
		{
			throw new net.jadoth.meta.NotImplementedYetError(); // FIX-ME ConstHashTable.Keys#view()
		}
		...
		{
			return elements.applies(ConstHashTable.this.predicateContainsKey());
		}
		...
		public final <T> T[] copyTo(final T[] target, final int offset)
		{
			ConstHashTable.this.chain.keyCopyToArray(0, ConstHashTable.this.size, target, offset);
			return target;
		}
		...
		{
			throw new net.jadoth.meta.NotImplementedYetError(); // FIXME ConstHashTable.Keys#old()
		}
	...
	public final class Values implements XImmutableTable.Values<K, V>
	{
		...
		{
			ConstHashTable.this.chain.valuesCopyToArray(0, Jadoth.to_int(this.size()), target, targetOffset);
			return target;
		}
		...
		{
			throw new net.jadoth.meta.NotImplementedYetError(); // FIXME ConstHashTable.Values#old()
		}
		...
		@Override
		public final XImmutableList<V> view(final int fromIndex, final int toIndex)
		{
			throw new net.jadoth.meta.NotImplementedYetError(); // FIXME ConstHashTable.Values#view()
		}
		...
		public final <T> T[] copyTo(final T[] target, final int targetOffset, final int offset, final int length)
		{
			ConstHashTable.this.chain.valuesCopyToArray(offset, length, target, targetOffset);
			return target;
		}
	...
	public final class OldVarMap implements XImmutableTable.Bridge<K, V>
	{
		...
		{
			throw new net.jadoth.meta.NotImplementedYetError(); // FIXME ConstHashTable.OldVarMap#keySet()
		}
