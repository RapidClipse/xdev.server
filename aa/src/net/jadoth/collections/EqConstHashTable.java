...
implements XImmutableTable<K,V>, HashCollection<K>, Composition
{
	...
	)
	{
		...
		entries.iterate(new Procedure<KeyValue<K,V>>() {
			@Override
	...
	final int internalRehash()
	{
		...
		{
			// (13.01.2015 TM)FIXME: /!\ DEBUG
			final int hash;
			try
			{
				hash = hashEqualator.hash(entry.key);
			}
...
//			final int hash = hashEqualator.hash(entry.key);
	...
	{
		throw new net.jadoth.meta.NotImplementedYetError(); // FIXME EqConstHashTable#old()
	}
	...
	public final Predicate<KeyValue<K,V>> predicateContainsEntry()
	{
		return new Predicate<KeyValue<K,V>>(){ @Override public boolean test(final KeyValue<K,V> entry) {
			...
			if((kv = EqConstHashTable.this.getEntry(entry.key())) == null) return false;

			// equality of values is architectural restricted to simple referential equality
			return EqConstHashTable.this.hashEqualator.equal(kv.key(), entry.key()) && kv.value() == entry.value();
		}};
	}
	...
	{
		throw new net.jadoth.meta.NotImplementedYetError(); // FIX-ME EqConstHashTable.Entries#range()
	}

	@Override
	public final XGettingEnum<KeyValue<K,V>> view(final int lowIndex, final int highIndex)
	{
		throw new net.jadoth.meta.NotImplementedYetError(); // FIX-ME EqConstHashTable.Entries#view()
	}
	...
	{
		return EqConstHashTable.this.chain.applies(predicate);
	}
	...
	public final class Keys implements XImmutableTable.Keys<K, V>, HashCollection<K>
	{
		...
		{
			throw new net.jadoth.meta.NotImplementedYetError(); // FIX-ME EqConstHashTable.Keys#range()
		}

		@Override
		public final XGettingEnum<K> view(final int lowIndex, final int highIndex)
		{
			throw new net.jadoth.meta.NotImplementedYetError(); // FIX-ME EqConstHashTable.Keys#view()
		}
		...
		public final boolean hasDistinctValues(final Equalator<? super K> equalator)
		{
			...
			throw new net.jadoth.meta.NotImplementedYetError(); // FIX-ME EqConstHashTable.Keys#hasDistinctValues()
		}
		...
		{
			throw new net.jadoth.meta.NotImplementedYetError(); // FIXME EqConstHashTable.Keys#old()
		}
	...
	public final class Values implements XImmutableTable.Values<K, V>
	{
		...
		{
			throw new net.jadoth.meta.NotImplementedYetError(); // FIXME EqConstHashTable.Values#old()
		}
		...
		{
			throw new net.jadoth.meta.NotImplementedYetError(); // FIXME EqConstHashTable.Values#view()
		}
	...
	public final class OldVarMap implements XImmutableTable.Bridge<K, V>
	{
		...
		{
			throw new net.jadoth.meta.NotImplementedYetError(); // FIXME EqConstHashTable.OldVarMap#keySet()
		}
