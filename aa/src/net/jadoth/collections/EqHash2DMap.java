...
public final class EqHash2DMap<K1, K2, V> implements X2DMap<K1, K2, V>, Composition
{
	...
	public static final <K1, K2, V> EqHash2DMap<K1, K2, V> New()
	{
		return new EqHash2DMap<>(
			JadothHash.<K1>hashEqualityIdentity(),
			JadothHash.<K2>hashEqualityIdentity()
		);
	}
	...
	{
		super();
		this.k1HashEqualator = coalesce(k1HashEqualator, JadothHash.<K1>hashEqualityIdentity());
		this.k2HashEqualator = coalesce(k2HashEqualator, JadothHash.<K2>hashEqualityIdentity());
	...
	@Override
	public final <P extends Procedure<? super KeyValue<K1, ? extends XGettingMap<K2, V>>>> P iterate(final P procedure)
	{
	...
	@Override
	public final <PK1 extends Procedure<? super K1>> PK1 iterateKeys1(final PK1 procedure)
	{
		return this.tree1.keys().iterate(procedure);
	}

	@Override
	public final <PK2 extends Procedure<? super K2>> PK2 iterateKeys2(final PK2 procedure)
	{
		this.tree1.values().iterate(new Procedure<EqHashTable<K2, V>>() {
			@Override
	...
	@Override
	public final <PV extends Procedure<? super V>> PV iterateValues(final PV procedure)
	{
		this.tree1.values().iterate(new Procedure<EqHashTable<K2, V>>() {
			@Override
			public void accept(final EqHashTable<K2, V> tree2)
	...
	@Override
	public <PIE extends Procedure<? super KeyValue<K2, V>>> PIE iterateInnerEntries(final PIE procedure)
	{
		this.tree1.values().iterate(new Procedure<EqHashTable<K2, V>>() {
			@Override
			public void accept(final EqHashTable<K2, V> tree2)
