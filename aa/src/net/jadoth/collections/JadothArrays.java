...
public final class JadothArrays
{
	 ...
	 * <p>
	 * Note that specific equality of each element is situational and thus cannot be a concearn
	 * of a generic array comparison, just as it cannot be the concearn of the element's class directly.
	 *
	 * @param array1
	 * @param array2
	 * @return
		...
		throws ArrayIndexOutOfBoundsException
	{
		...
		for(int i = base; i < bound;){
			while(i < bound && array[i] == e){ // determine move offset (next occurance of non-e)
				i++;
			}
			final int moveOffset = i;

			while(i < bound && array[i] != e){ // determine move length (range until next e)
				i++;
			}
	...
	static final class ArrayElementRemover<E> implements Procedure<E>
	{
		...
		throws ArrayIndexOutOfBoundsException
	{
		...
		// compress array by moving non-subjects block by block
		for(int i = base; i < bound;){
			while(i < bound && equalator.equal(array[i], e)){ // determine move offset (next occurance of non-e)
				i++;
			}
			final int moveOffset = i;

			while(i < bound && !equalator.equal(array[i], e)){ // determine move length (range until next e)
				i++;
			}
			final int moveLength = i - moveOffset;
		...
		throws ArrayIndexOutOfBoundsException
	{
		...
		while(currentMoveTargetIndex < bound && !elements.containsSearched(JadothPredicates.predicate(array[currentMoveTargetIndex], equalator))){
	...
	public static <E, C extends Procedure<? super E>> C filterTo(
		final E[] elements,
		final C target,
	...
	)
	{
		...
		for(final E e : elements) if(predicate.test(e)) target.accept(e);
		return target;
	}
		...
		throws IllegalArgumentException
	{
		...
		for(int i = indicesOffset; i < indicesBound; i++){
			if(indices[i] >= targetLength){ // indices < 0 explicitely valid for allowing items to be skipped
	...
	)
	{
		...
		main: for(int ei = elementsLowOffset; ei < elementsHighBound; ei++){
	...
	@SafeVarargs
	public static <E> void iterate(final Procedure<? super E> iterator, final E... elements)
	{
	...
	public static <E> void iterate(final Procedure<? super E> iterator, final E[] elements, final int offset, final int length)
	{
	...
	private JadothArrays() { throw new UnsupportedOperationException(); } // static only
}
