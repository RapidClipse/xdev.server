...
public final class JadothSort
{
	...
	public static final transient Comparator<Boolean> compareBoolean = new Comparator<Boolean>(){
		@Override public int compare(final Boolean b1, final Boolean b2){
			if(b1 == null){
				return b2 == null ?0 :-1; // null is smaller than everyhing
			}
			else if(b2 == null){
				return 1; // null is smaller than everyhing
			}
			return !b1.booleanValue() && b2.booleanValue() ?-1 :!b1.booleanValue() && !b2.booleanValue() ?1 :0;
		}
	};

	public static final transient Comparator<Byte> compareByte = new Comparator<Byte>(){
		@Override public int compare(final Byte b1, final Byte b2){
			if(b1 == null){
				return b2 == null ?0 :-1; // null is smaller than everyhing
			}
			else if(b2 == null){
			...
			return b2 >= b1 ? b2 != b1 ? -1:0:+1;
		}
	};

	public static final transient Comparator<Short> compareShort = new Comparator<Short>(){
		@Override public int compare(final Short s1, final Short s2){
			if(s1 == null){
				return s2 == null ?0 :-1; // null is smaller than everyhing
			}
			else if(s2 == null){
				return 1; // null is smaller than everyhing
			}
			return s2 >= s1 ? s2 != s1 ? -1:0:+1;
		}
	};

	public static int compareInteger(final Integer i1, final Integer i2)
	{
		if(i1 == null){
			return i2 == null ?0 :-1; // null is smaller than everyhing
		}
		else if(i2 == null){
			return 1; // null is smaller than everyhing
		}
		return i2 >= i1 ? i2 != i1 ? -1:0:+1;
	}

	public static final transient Comparator<Long> compareLong = new Comparator<Long>(){
		@Override public int compare(final Long l1, final Long l2){
			if(l1 == null){
				return l2 == null ?0 :-1; // null is smaller than everyhing
			}
			else if(l2 == null){
				return 1; // null is smaller than everyhing
			}
			return l2 >= l1 ? l2 != l1 ? -1:0:+1;
		}
	};

	public static final transient Comparator<Float> compareFloat = new Comparator<Float>(){
		@Override public int compare(final Float f1, final Float f2){
			if(f1 == null){
				return f2 == null ?0 :-1; // null is smaller than everyhing
			}
			else if(f2 == null){
				return 1; // null is smaller than everyhing
			}
			return f2 >= f1 ? f2 != f1 ? -1:0:+1;
		}
	};

	public static final transient Comparator<Double> compareDouble = new Comparator<Double>(){
		@Override public int compare(final Double d1, final Double d2){
			if(d1 == null){
				return d2 == null ?0 :-1; // null is smaller than everyhing
			}
			else if(d2 == null){
				return 1; // null is smaller than everyhing
			}
			return d2 >= d1 ? d2 != d1 ? -1:0:+1;
		}
	};

	public static final transient Comparator<Character> compareCharacter = new Comparator<Character>(){
		@Override public int compare(final Character c1, final Character c2){
			if(c1 == null){
				return c2 == null ?0 :-1; // null is smaller than everyhing
			}
			else if(c2 == null){
				return 1; // null is smaller than everyhing
			}
			return c2 >= c1 ? c2 != c1 ? -1:0:+1;
		}
	};

	public static final transient Comparator<String> compareStringLength = new Comparator<String>(){
		@Override public int compare(final String s1, final String s2){
			if(s1 == null){
				return s2 == null ?0 :-1; // null is smaller than everyhing
			}
			else if(s2 == null){
			...
			return s2.length() >= s1.length() ? s2.length() != s1.length() ? -1:0:+1;
		}
	};
	...
	private static transient int r = 0;
	static {
		initPseudoRandom();
	}
	...
	private static <E> E[] mergesortCache(final E[] buffer, final E[] values)
	{
		...
		if(buffer == values){
			throw new IllegalArgumentException("buffer may not be identical to values array");
		}
	...
	static <E> void adaptiveMergesort(
		final E[]                   buffer,
		final E[]                   values,
		final int                   start ,
		final int                   bound ,
	...
	{
		if(bound - start < 7){ // length of 7 or less (bound - start + 1 < 8)
		...
		if(log2 > 6){ // big range part costs 1-3% performance for random data
		...
		else { // low range part costs additional 4-7% performance for random data (because executed much more often)
	...
	public static <E> void parallelSort(final E[] elements, final Comparator<? super E> comparator)
	{
		...
		if(elements.length < 8192){ // identical to simple sort() for small length
			adaptiveMergesort(elements.clone(), elements, 0, elements.length, comparator, log2(elements.length));
			return;
		}
		...
		new Thread(){ @Override public void run(){
			...
			board[1] = true;
			if(board[0]){ // if main thread was faster (and waits now), notify it to proceed
				synchronized(board) {
					board.notify();
				}
			}
		...
		board[0] = true;
		if(!board[1]){ // if second thread is not done yet, wait for it
			synchronized(board) {
				try {
					board.wait();
				}
...
//	private static <E> boolean tryDistinctsort10Into(
//		final E[] values,
//		final E[] target,
//		final int start,
//		final int bound,
...
//	)
//	{
//		E last0, last1, last2, /*last3, last4, last5, last6, last7, last8, last9,*/ current
//		= last0 =last1 =last2 /*=last3 =last4 =last5 =last6 =last7 =last8 =last9*/ =values[start];
//		int distinctCount = 0;
//
//		for(int i = start+1; i < bound; i++){
//			if(comparator.compare(current, current = values[i]) == 0
//			|| comparator.compare(current, last0) == 0
...
////			|| comparator.compare(current, last8) == 0
////			|| comparator.compare(current, last9) == 0
//			) continue;
//
//
//			if(++distinctCount == 5){
...
////			case  9: target[bound -10] = last9;
////			case  8: target[bound - 9] = last8;
////			case  7: target[bound - 8] = last7;
////			case  6: target[bound - 7] = last6;
////			case  5: target[bound - 6] = last5;
////			case  4: target[bound - 5] = last4;
////			case  3: target[bound - 4] = last3;
//			case  2: target[bound - 3] = last2;
//			case  1: target[bound - 2] = last1;
//			case  0: target[bound - 1] = last0;
//		}
//		distinctCount++;
//
//		// phase 2: sort distinct values (outperforms a quicksort call even for length 10)
//		for(int i = distinctCount = bound - distinctCount; i < bound; i++){
//			for(int j = i; j > distinctCount && comparator.compare(target[j-1], target[j]) > 0; j--){
//				swap(target, j, j-1);
//			}
//		}
//
//		// phase 3: copy source values according to sortation of distinct values(distincts get flushed correctly)
//		for(int targetIndex = 0; distinctCount < bound; distinctCount++){
//			current = target[distinctCount];
//			for(int i = start; i < bound; i++){
//				if(comparator.compare(current, values[i]) == 0){
//					target[targetIndex] = values[i];
//					targetIndex++; // incrementing afterwards makes algorithm 10% faster for range < 1000 ^^
//				}
//			}
//		}
//		return true;
//	}
	...
	{
		if(high - low < 31){ // note TM: removed length variable as it is only used 3 times.
		...
		if((pivot1 = a[e2]) !=(pivot2 = a[e4])){
			a[e2] = a[low];
			a[e4] = a[high];
			while(a[++left] < pivot1){/*_*/}
			while(a[--right] > pivot2){/*_*/}
			outer:
	...
	)
	{
		...
		if(high - low < 16){ // note TM: sentinel stuff and length 32 for objects turned out to be slower than this
		...
		if(cmp.compare(a[e3], a[e2]) < 0){
			...
			if(cmp.compare(t, a[e1]) < 0){ a[e2] = a[e1]; a[e1] = t; }
		}
		...
		if(cmp.compare(pivot1 = a[e2], pivot2 = a[e4]) != 0){
			a[e2] = a[low];
			a[e4] = a[high];
			while(cmp.compare(a[++left], pivot1) < 0){/*_*/}
			while(cmp.compare(a[--right], pivot2) > 0){/*_*/}
			outer:
			for(int k = left; k <= right; k++){
				final E ak = a[k];
	...
	/**
	 * Sorting order evaluation helper method for source code simplification where performance is not a (big) concearn.
	 * <p>
	 ...
	 * performance is no big concearn (e.g. not many elements to sort, not many values to compare, fast value comparisons)
	 *
...
//	static int comps = 0;
//	static boolean comp(){ comps++; return true;}
//
//	static int swaps = 0;
//	static boolean swap(){ swaps++; return true;}
//
