...
public class MappedList<E, S> implements XGettingList<E>
{
	...
	private Comparator<S> mapComparator(final Comparator<? super E> comparator)
	{
		return new Comparator<S>(){
			@Override public int compare(final S o1, final S o2) {
	...
	private Predicate<S> mapPredicate(final Predicate<? super E> predicate)
	{
		return new Predicate<S>(){
			@Override public boolean test(final S e) {
	...
	public <T extends Procedure<? super E>> T copySelection(final T target, final int... indices)
	{
		...
		for(int i = 0, idx; i < length; i++){
			if((idx = indices[i]) < 0 || idx >= size){
				throw new IndexExceededException(size, idx);
			}
		}
	...
	public <T> T[] copyTo(final T[] target, final int targetOffset, final int offset, final int length)
	{
		...
		XUtilsCollection.rngIterate(this.subject, offset, length, new Procedure<S>() {
			int toff = targetOffset;
			@SuppressWarnings("unchecked")
			@Override public void accept(final S e) {
...
//	}
	...
	@Override
	public XGettingList<E> view(final int lowIndex, final int highIndex)
	{
		throw new net.jadoth.meta.NotImplementedYetError(); // FIX-ME XGettingList<E>#view
	}

	@Override
	public XGettingList<E> range(final int fromIndex, final int toIndex)
	{
