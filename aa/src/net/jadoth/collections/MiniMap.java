...
public final class MiniMap<K,V> implements Composition
{
	...
	public MiniMap(final int initialCapacity, final KeyValue<? extends K, ? extends V>... data)
	{
		...
		for(int i = 0; i < data.length; i++){
			...
			slots[identityHashCode(key) & modulo] = new Entry<>(key, entry.value(), slots[identityHashCode(key) & modulo]);
		}
	}
	...
	MiniMap(final int size, final ConstMiniMap.Entry<K, V>[] source)
	{
		...
		for(int i = 0; i < source.length; i++){ // iterate through all entries and assign them to the new storage
	...
	private void rebuildStorage(final int newSlotLength)
	{
		...
		for(int i = 0; i < slots.length; i++){ // iterate through all entries and assign them to the new storage
	...
	public V put(final K key, final V value)
	{
		...
		this.slots[identityHashCode(key) & this.modulo] = new Entry<>(key, value, this.slots[identityHashCode(key) & this.modulo]);
	...
	public MiniMap<K,V> putAll(@SuppressWarnings("unchecked") final KeyValue<K,V>... data)
	{
		...
		for(int i = 0; i < data.length; i++){ // iterate through all entries and assign them to the new storage
			final K key;
			if((key = data[i].key()) == null){
				continue;
			}
			slots[identityHashCode(key) & modulo] = new Entry<>(key, data[i].value(), slots[identityHashCode(key) & modulo]);
			if(this.size++ >= modulo){
				this.increaseStorage(); // storage has to be increased, all entries have to be transfered to new storage
				slots = this.slots;
				modulo = this.modulo;
