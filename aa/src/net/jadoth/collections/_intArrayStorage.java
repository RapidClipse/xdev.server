 ...
 * @author Thomas Muenz
 */
public abstract class _intArrayStorage
{
	...
	)
	{
		...
		main: for(int ei = elementsOffset, bound = elementsOffset+elementsLength; ei < bound; ei++){
	...
	)
	{
		...
		main: for(int ei = 0; ei < eSize; ei++){
			final int element = eData[ei];
			for(int di = offset-d; di != endIndex;){
				if(element == data[di+=d]) continue main;
			}
			return false;  // one element was not found in this list, return false
		}
		return true;  // all elements have been found, return true
	...
	{
		// trying to optimize f�r bulk procedure types here is hardly possible due to predicate
	...
	)
	{
		...
		if(target instanceof _intList){
			...
			throw new net.jadoth.meta.NotImplementedYetError(); // FIXME _intArrayStorage#rngCopyTo()
...
//			return target;
	...
	)
	{
		...
		for(int i = 0; i < length; i++){
			target.add(data[indices[i]]); // manual loop to spare temporary variable
		}
	...
	)
	{
		...
		try {
			for(int i = 0, idx; i < length; i++){
				target.add(data[idx = indices[i]]); // manual loop to spare temporary variable
	...
	{
		return ascending ?isSortedAscending(data, size) :isSortedDescending(data, size);
	}
