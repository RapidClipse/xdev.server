...
public final class _intSet implements Composition
{
	...
	private int size, range, capLower, capUpper, lines[][];
	private boolean has0; // 0 is treated as "null" in algorithms, so 0 as value has to be specialcased.
	...
	private _intSet(final int uncheckedInitialCapacity)
	{
		...
		this.capUpper = uncheckedInitialCapacity >= 1<<30 ?Integer.MAX_VALUE :uncheckedInitialCapacity;
	...
	private void internalRebuildStorage(final int newLength)
	{
		...
		this.capUpper = newLength == 1<<30 ?Integer.MAX_VALUE :newLength;
	}
	...
	{
		if(value == 0){ // case: 0 value
			return this.internalAdd0();
		}

		final int[] line;
		if((line = this.lines[value & this.range]) == null){ // case: no hash line at all, yet
			this.internalAddNewLine(value);
			return true;
		}
		for(final int i : line) {
			if(i == value){ // case: value already contained
				return false;
			}
			if(i == 0){ // case: new value and hash line has still enough room
	...
	public final String toString()
	{
		...
		final VarString vs = VarString.New(JadothMath.capInt(this.size * 4L)).add('[');
		if(this.has0){
			vs.add(0).add(',');
		}
		for(final int[] line : this.lines){
	...
	public static void main(final String[] args)
	{
		...
		System.out.println(New(1, 2, 3, 4));
		System.out.println(New(1, 2, 2, 3, 4));
		System.out.println(New(0));
		System.out.println(New(0, 1));
		System.out.println(New(0, 1, 2, 3, 4));
		System.out.println(New(0, 1, 2, 2, 3, 4));
		System.out.println(New(0, 1, 2, 2, 3, 4).contains(3));
		System.out.println(New(0, 1, 2, 2, 3, 4).contains(5));
		System.out.println(Arrays.toString(New(0, 1, 2, 2, 3, 4, 4, 4, 4).toArray()));

		final _intSet ints = New(0, 1, 2, 2, 3, 4);
		ints.remove(3);
		...
		final int sum = New(0, 1, 2, 2, 3, 4).iterate(new _intSum()).yield();
