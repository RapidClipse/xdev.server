...
public interface ParallelProcedure<E> extends Procedure<E>
{
	/**
	...
	/**
	 * Type providing the actual logic {@link Procedure} to be applied to the elements enountered by a
	 ...
	 */
	public interface LogicProvider<S, P extends Procedure<? super S>>
	{
		/**
		 * Provides the actual logic {@link Procedure} to be applied to the elements enountered by a
		...
		public final class SingletonLogic<S, P extends Procedure<? super S>> implements LogicProvider<S, P>
		{
	 ...
	 * Also, any other {@link Throwable} thrown by the logic {@link Procedure} will cause the throwing worker thread
	...
	public final class Implementation<E> implements ParallelProcedure<E>
	{
		...
		private static final ThreadTimeoutProvider DEFAULT_THREAD_TIMEOUT = new ThreadTimeoutProvider(){
			@Override public int threadTimeout() {
				return 1000; // ms, as defined by System.currentTimeMillis()
			}
		};
		...
		private final LogicProvider<? super E, ? extends Procedure<? super E>> logicProvider;
		...
		private Entry<E> head = null, tail = null;
		...
		public Implementation(
			final LogicProvider<? super E, ? extends Procedure<? super E>> logicProvider,
			final int threadCount
		)
		{
			this(logicProvider, new ThreadCountProvider.Constant(threadCount), DEFAULT_THREAD_TIMEOUT);
		}
		...
		)
		{
			...
			this.threadTimeoutProvider = coalesce(threadTimeout, DEFAULT_THREAD_TIMEOUT);
		...
		private void checkWorkerCreation()
		{
			...
			if(Jadoth.to_int(this.threads.size()) < this.threadCountProvider.maxThreadCount()){
				this.threads.add(start(createWorkerThread()));
			}
		}
		...
		{
			synchronized(this.threads) { // don't disturb worker threads via this, but synchronize thread cache
		...
		// (19.03.2014 TMuenz)TODO: fix unrelated two S type parameters in LogicProvider
		@SuppressWarnings("unchecked")
		final <S, E1 extends S, P extends Procedure<? super S>> void runWorker(final WorkerThread worker)
		{
		...
		static <S, E extends S, P extends Procedure<? super S>> void delegateRun(
