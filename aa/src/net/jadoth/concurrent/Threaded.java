...
public class Threaded<E> implements ConsolidatableCollection, OptimizableCollection, Sized, Referencing<E>
{
	...
	private static <E> Entry<E>[] createSlots(final int minimumLength)
	{
		...
		if(minimumLength >= 1<<30){
			return new Entry[1<<30]; // technical magic number
		}
	...
	private volatile Procedure<E> cleanUpOperation;
	...
	public Procedure<E> getCleanUpOperation()
	{
	...
	public Threaded<E> setCleanUpOperation(final Procedure<E> cleanUpOperation)
	{
	...
	public E set(final E element)
	{
		...
		for(Entry<E> e = (slots=this.slots)[identityHashCode(currentThread()) & slots.length-1]; e != null; e = e.next){
			if(e.get() == currentThread()){ // repeated currentThread() proved to be faster than variable
				final E old = e.value;
				e.value = element;
				return old;
			}
	...
	public E remove()
	{
		...
		for(Entry<E> e = (slots=this.slots)[identityHashCode(currentThread()) & slots.length-1]; e != null; e = e.next){
			if(e.get() == currentThread()){ // can't directly remove found entry as storeage could have been rebuilt.
	...
	private synchronized void removeForCurrentThread(final int threadIdHashCode)
	{
		...
		if((head = (slots = this.slots)[threadIdHashCode & slots.length-1]) != null){
			if(head.get() == currentThread()){ // head entry special case
	...
	private int internalOptimize()
	{
		...
		final Procedure<E> cleanUpOp = this.cleanUpOperation;
		int count = 0;
		 ...
		 *   immutably and only their exclusive entry concearns them, but neither the newly created one for the current
