...
public final class JadothMath
{
	...
	private static final transient int MAX_POW_2_INT = 1<<30; // highest value that can be achieved by leftshifting 1.
	...
	private static final double internalRound(final double value, int decimals)
	{
	...
	public static int[] randoming(int length)
	{
		...
		while(length --> 0){
			ints[length] = random.nextInt(bound);
		}
		return ints;
	}

	public static int[] randoming(int length, final int bound)
	{
		final int[] ints = new int[length];
		while(length --> 0){
			ints[length] = random.nextInt(bound);
	...
	public static int[] randoming(int length, final int lowestValue, final int bound)
	{
		final int[] ints = new int[length];
		final int randomBound = bound - lowestValue;
		while(length --> 0){
			ints[length] = random.nextInt(randomBound) + lowestValue;
		}
		return ints;
	}
...
//		return result;
	...
	private JadothMath() { throw new UnsupportedOperationException(); } // static only
}
