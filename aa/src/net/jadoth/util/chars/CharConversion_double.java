...
public final class CharConversion_double
{
	...
	private static int handle_doubleGte1DenormSpecialCase(final char[] target, final int offset, int exponent)
	{
		...
		target[offset + exponent] = '.';
		target[offset + exponent + 1] = '0';
		return offset + exponent + 2;
	}
	...
	private static int removeTrailingLast(final char[] target, final int offset)
	{
		...
		if(target[offset - 2] == '9' && target[offset - 1] >= '8'){ // '8' because 16th digit can be off by +/- 1.
			return removeTrailingNinesSimple(target, offset - 2);
		}
		if(target[offset - 2] == '0' && target[offset - 1] <= '1'){ // 1 because 16th digit can be off by +/- 1
			return removeTrailingZerosSimple(target, offset - 2);
		}
		if(target[offset - 1] == '9' && target[offset] >= '5'){ // simple rounding for 17th digit
			target[offset - 2]++;
			return offset - 1;
		}
		if(target[offset - 1] == '0' && target[offset] < '5'){ // simple rounding for 17th digit
			return offset - 1;
		}
		if(target[offset] >= '5'){ // simple rounding for 17th digit
			target[offset - 1]++;
			return offset;
		}
		return offset;
	...
	private static int removeTrailingPreLast(final char[] target, final int offset)
	{
		...
		if(target[offset - 1] == '9' && target[offset] >= '8'){ // '8' because 16th digit can be off by +/- 1.
			return removeTrailingNinesSimple(target, offset - 1);
		}
		if(target[offset - 1] == '0' && target[offset] <= '1'){ // 1 because 16th digit can be off by +/- 1
			return removeTrailingZerosSimple(target, offset - 1);
		}
		if(target[offset] >= '8'){ // 8 because 16th digit can be off by +/- 1
			target[offset - 1]++;
			return offset;
		}
		if(target[offset] <= '1'){ // 1 because 16th digit can be off by +/- 1
			return offset;
		}
		return offset + 1;
	}
	...
	private CharConversion_double() { throw new UnsupportedOperationException(); } // static only
}
