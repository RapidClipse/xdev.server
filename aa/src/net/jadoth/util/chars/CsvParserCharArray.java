...
public final class CsvParserCharArray implements CsvParser<_charArrayRange>, Stateless
{
	...
	private static final _charRangeProcedure dummyCollector = new _charRangeProcedure(){
	...
	)
	{
		...
		while(++i < iBound) // preincrement to skip the opening delimiter right away
		{
			// handle escaping
			if(input[i] == escaper){
				if(i + 1 == iBound){ // damn special cases
					break;
				}
	...
	)
	{
		...
		{
			return CsvRecordParserCharArray.Static.skipSkippable(input, iStart + 1, iBound, config.commentSignal(), config);
		}
		int i = iStart;
...
//	private static int seekSegmentIdentifier(
//		final char[]             input         ,
//		final int                iStart        ,
//		final int                iBound        ,
//		final VarString          literalBuilder,
//		final CsvConfiguration   config        ,
//		final XReference<String> segmentName
//	)
//	{
//		final char segmentStarter   = config.segmentStarter();
...
//			if(i >= iBound)
//			{
//				return i; // abort at end of data, consistent state of segmentName is irrelevant, gets discarded anyway
//			}
//			if(input[i] == segmentStarter)
//			{
//				i++; // skip comment starter and abort
//				break; // don't return but fall through to segmentName setting
//			}
//			else if(input[i] == literalDelimiter)
//			{
//				i = parseLiteralDelimited(
//					input                   ,
//					i                       ,
...
//					dummyCollector
//				);
//				if(segmentName.get() == null)
//				{
//					segmentName.set(literalBuilder.toString());
//				}
//				literalBuilder.reset();
//			}
//			else if(input[i] == commentSignal)
//			{
//				i = CsvRecordParserCharArray.Static.skipComments(input, i, iBound, config);
//			}
//			else
//			{
//				literalBuilder.add(input[i++]);
...
//		}
//
//		// set segment name if (still) applicable at this point
//		if(segmentName.get() == null)
//		{
//			literalBuilder.trim();
...
//	}
	...
	)
	{
		...
		// skip any comments
		i = CsvRecordParserCharArray.Static.skipDataComments(input, i, iBound, terminator, valueSeparator, recordSeparator, commentSignal, config);

		if(i < iBound && input[i] == config.headerStarter())
		{
			// i + 1 to skip the header starter/terminator characters
			i = parseSimpleColumnLine(input, i + 1, iBound, config.headerTerminator(), terminator, config, literalBuilder, columnTypes);

			if(i >= iBound || input[i] == terminator)
			{
				throw new RuntimeException("Unclosed header type line at index "+i); // (22.11.2014)EXCP: proper exception
			}
	...
	public CsvConfiguration parseCsvData(
		...
		final _charArrayRange                             input         ,
		      CsvSegmentsParser.Provider<_charArrayRange> parserProvider,
		final CsvRowCollector                             rowAggregator
	)
	{
		// defaulting logic
		if(config == null)
		{
			config = CSV.configurationDefault();
		}
		if(parserProvider == null)
		{
			parserProvider = CsvParserCharArray::provideSegmentsParser;
		}
		...
		final int boundIndex = skipLinesBackwards(
			...
			input.bound(),
			config.recordSeparator(),
			config.trailingLineCount()
		);

		// note that line skipping intentionally cares only for pure line separators, no delimited symbols or such
		int i = skipLines(data, startIndex, boundIndex, config.recordSeparator(), config.skipLineCount());

		// skip all skippable (whitespaces and comments by passed/default config) at the beginning.
		i = CsvRecordParserCharArray.Static.skipSkippable(data, i, boundIndex, config.commentSignal(), config);
		...
		final CsvSegmentsParser<_charArrayRange> parser = parserProvider.provideSegmentsParser(config, rowAggregator);
	...
	static final class ColumnHeaderCollector implements _charRangeProcedure
	{
		...
		public final void accept(final char[] data, final int offset, final int length)
		{
			this.values.add(new String(data, offset, length));
		}
