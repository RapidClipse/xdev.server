...
public interface CsvRecordParserCharArray
{
	...
	{
		public static boolean isTrailingSeparator(final char[] input, final int iLowBound, int i, final char separator)
		{
			while(i --> iLowBound)
			{
		...
		)
		{
			...
			while(i < iBound)
			{
				...
				{
					return i; // nasty special case: single trailing comment signal character (but no comment) at the end
				}
		...
		)
		{
			...
			while(i < iBound)
			{
				// check control characters explicitely before skipping white spaces as they might be whitespaces themselves.
		...
		)
		{
			...
			while(i < iBound)
			{
				 ...
				 * so no configuration.
				 * Simplified check for just \n is intentional. It covers both standard/java/unix and windows's clumsy \r\n.
		...
		)
		{
			...
			throw new RuntimeException("Incomplete trailing full comment at index "+i); // (22.11.2014)EXCP: proper exception
		}
