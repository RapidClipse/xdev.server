...
public interface TimeSpan
{
	...
	public class Format
	{
		...
		{
			return assembleFormat(VarString.New(this.formatString.length() + 20), timeSpan).toString();
		}
	...
	public final class Implementation implements TimeSpan
	{
		...
		private transient int[] array = null;
		...
		public String toString()
		{
			...
			if((value = this.days) != 0 || valueFound){
				vc.padLeft(Integer.toString(value), 3, ' ').add('d', ' ');
				valueFound = true;
			}
			if((value = this.hours) != 0 || valueFound){
//				padLeft(vc, Integer.toString(value), 2)
				vc.padLeft(Integer.toString(value), 2, ' ').add('h', ' ');
				valueFound = true;
			}
			if((value = this.minutes) != 0 || valueFound){
//				padLeft(vc, Integer.toString(value), 2)
				vc.padLeft(Integer.toString(value), 2, ' ').add('m', ' ');
				valueFound = true;
			}
			if((value = this.seconds) != 0 || valueFound){
//				padLeft(vc, Integer.toString(value), 2)
				vc.padLeft(Integer.toString(value), 2, ' ').add('s', ' ');
				valueFound = true;
			}
			vc.padLeft(Integer.toString(this.millis), 3, ' ').add('m', 's', ' ');
