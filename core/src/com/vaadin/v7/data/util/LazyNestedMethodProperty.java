...
public class LazyNestedMethodProperty<T> extends AbstractProperty<T>
{
	 ...
	 *            the output stream
	 * @throws IOException
	 *             if exception occurs in serialization
	 */
	 ...
	 *            the input stream
	 * @throws IOException
	 *             if IO exception occurs in read
	 * @throws ClassNotFoundException
	 *             if class not found exception occur in read
	 */
	...
	private void initialize(final Class<?> beanClass, final String propertyName)
	{
		...
		{
			final String simplePropertyName = simplePropertyNames[i].trim();
			if(simplePropertyName.length() > 0)
			{
				...
				try
				{
					final Method getter = MethodProperty.initGetterMethod(simplePropertyName,
							propertyClass);
				...
				catch(final java.lang.NoSuchMethodException e)
				{
					throw new IllegalArgumentException(
							"Bean property '" + simplePropertyName + "' not found",e);
				}
			}
			else
			{
	...
	public T getValue()
	{
		try
		{
			...
			for(final Method m : this.getMethods)
			{
	...
	protected void invokeSetMethod(final T value)
	{
		try
		{
			...
			{
				object = this.getMethods.get(i).invoke(object);
				if(object == null)
				{
					return;
				}
	 ...
	 *
	 * @return unmodifiable list of getter methods corresponding to each segment of
	 *         the property name
	 */
