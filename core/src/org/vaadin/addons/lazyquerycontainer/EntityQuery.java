...
public class EntityQuery<E> implements Query, Serializable
{
	...
	public final int size()
	{
		...
		if(this.querySize == -1)
		{
			...
			final javax.persistence.Query query = this.entityManager.createQuery(cq);
	...
	public final List<Item> loadItems(final int startIndex, final int count)
	{
		...
		final javax.persistence.TypedQuery<E> query = this.entityManager.createQuery(cq);
			...
			final CriteriaQuery<?> cq, final Root<?> root)
	{
		...
		if(filter instanceof Compare)
		{
			...
			final Expression<Comparable> property = (Expression)getPropertyPath(root,
