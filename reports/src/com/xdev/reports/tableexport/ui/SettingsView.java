...
public class SettingsView extends XdevView
{
	...
	public SettingsView(final XdevTable<?> table, final ExportType... availableExportTypes)
	{
		...
		this.tblColumnChooser.addContainerProperty("...",CheckBox.class,null);
		this.tblColumnChooser.addContainerProperty("Columnname",String.class,null);
		this.tblColumnChooser.addContainerProperty("Columnwidth",TextField.class,null);
		this.tblColumnChooser.addContainerProperty("Stretch",CheckBox.class,null);
		this.tblColumnChooser.addContainerProperty("ColumnId",Object.class,null);

		this.tblColumnChooser.setColumnExpandRatio("Columnname",1.0F);

		this.tblColumnChooser.setColumnAlignment("...",Table.Align.CENTER);
		this.tblColumnChooser.setColumnAlignment("Stretch",Table.Align.CENTER);
		this.tblColumnChooser.setColumnAlignment("Columnwidth",Table.Align.CENTER);
		...
		for(final Object columnId : this.table.getVisibleColumns())
		{
			...
			field.addStyleName("small, borderless");
			field.setConverter(new StringToIntegerConverter());
			field.setConvertedValue(0);
			field.addValidator(new IntegerRangeValidator("",0,null));
			field.setColumns(5);

			field.addValueChangeListener(event -> check());
			...
			if(this.table.getColumnWidth(columnId) == -1)
			{
				field.setConvertedValue(150);
				stretchBox.setValue(false);
			}
			else
			{
				field.setConvertedValue(this.table.getColumnWidth(columnId));
			}
			...
			this.tblColumnChooser.addItem(new Object[]{selectBox,
					this.table.getColumnHeader(columnId),field,stretchBox,columnId},++id);
		}
		
		this.tblColumnChooser.setVisibleColumns("...","Columnname","Columnwidth","Stretch");
	}
	...
	private void cbPapeOrientation_valueChange(final Property.ValueChangeEvent event)
	{
		this.exportSettings.setPageOrientation((PageOrientation)this.cbPageOrientation.getValue());
	...
	private int calculateFixedWidth()
	{
		...
		for(final Object object : getSelectedColumns())
		{
			...
			final int convertedValue = (Integer)((TextField)item.getItemProperty("Columnwidth")
	...
	private List<Object> getSelectedColumns()
	{
		...
		{
			final Item item = this.tblColumnChooser.getItem(object);
			if(((CheckBox)item.getItemProperty("...").getValue()).getValue())
			{
	...
	private List<Column> createColumns()
	{
		...
		for(final Object id : this.tblColumnChooser.getItemIds())
		{
			...
			final int columnWidth = Integer.valueOf(
					((TextField)item.getItemProperty("Columnwidth").getValue()).getValue());
			final String columnHeader = (String)item.getItemProperty("Columnname").getValue();
			float columnExpandRatio = 0F;
			if(((CheckBox)item.getItemProperty("...").getValue()).getValue() == true)
			{
			...
			final Object propertyId = item.getItemProperty("ColumnId").getValue();
	...
	private void cmdDeselectAll_buttonClick(final Button.ClickEvent event)
	{
		selectAll(false);
	}
	...
	private void selectAll(final boolean status)
	{
		...
		{
			final Item item = this.tblColumnChooser.getItem(object);
			((CheckBox)item.getItemProperty("...").getValue()).setValue(status);
		}
	}
	...
	private void initUI()
	{
		...
		this.tblColumnChooser.setCaption("Spalten Konfigurieren");
		this.verticalLayout4.setCaption(" ");
		this.cmdSelectAll.setCaption("Select all");
		this.cmdSelectAll.setStyleName("link small");
		this.cmdDeselectAll.setCaption("Deselect all");
		this.cmdDeselectAll.setStyleName("link small");
		...
		this.cmdDeselectAll.setSizeUndefined();
		this.verticalLayout4.addComponent(this.cmdDeselectAll);
		this.verticalLayout4.setComponentAlignment(this.cmdDeselectAll,Alignment.MIDDLE_LEFT);
		...
		this.cmdDeselectAll.addClickListener(event -> this.cmdDeselectAll_buttonClick(event));
		...
		this.cbPageOrientation.addValueChangeListener(new Property.ValueChangeListener()
		{
			...
			public void valueChange(final Property.ValueChangeEvent event)
			{
				SettingsView.this.cbPapeOrientation_valueChange(event);
			}
		});
	...
	private Button							cmdSelectAll, cmdDeselectAll;
