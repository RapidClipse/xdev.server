...
public interface FillTree
{
	...
	public class Implementation implements FillTree
	{
		...
		public void addGroup(final Class<?> clazz, final Field parentReference, final Field caption)
		{
			...
			for(final Iterator<Group> i = this.treeReferenceMap.keySet().iterator(); i.hasNext();)
			{
				final Group parentNode = i.next();
				if(parentNode.getGroupClass().equals(parentReference.getType()))
				{
		...
		public HierarchicalContainer fillTree(HierarchicalContainer container)
		{
			...
			{
				final Group node = i.next();
				container = addChildren(container,node);
			}
			return container;
		}
		...
		{
			for(final Iterator<Group> i = this.treeReferenceMap.values().iterator(); i.hasNext();)
			{
				final Group parent = i.next();
				if(parent == ROOT_IDENTIFIER)
				{
					for(final Iterator<Group> e = this.treeReferenceMap.keySet().iterator(); i
							.hasNext();)
					{
						final Group node = e.next();
